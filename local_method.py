...
class LocalMethod:
    ...
    def calc_constrains(self):
        ...
        eq3_constrains = np.zeros([self.grid_size, self.grid_size - 1])

        # self.u[:, 0] -= min(0, np.min(self.u[:, 0]))  # setting the minimium of  u's first row to zero
        for i in range(self.grid_size):
            for j in range(self.grid_size - 1):
                eq3_constrains[i, j] = self.S * (-self.pics[0][i, j] + self.pics[0][i, j + 1] - self.pics[2][i, j + 1])
                eq3_j_constrain = -self.u[i, j + 1] + self.u[i, j] + eq3_constrains[i, j]
                self.u[i, j + 1] += max(eq3_j_constrain, 0)
    ...
    def export_constrains_to_mesh(self):
        for i in range(self.grid_size + 1):
            for j in range(self.grid_size):
                self.create_wall_mesh(i, j, self.u[i, j])
                if i != self.grid_size:
                    self.create_receiver_mesh(i, j, self.r[i, j])
                    self.create_plus_chamfer(i, j, self.cplus[i, j])
                    self.create_minus_chamfer(i, j, self.cminus[i, j])
                    self.create_vwall_mesh(i, j, self.v[i, j])
